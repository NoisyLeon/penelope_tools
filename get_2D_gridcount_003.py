...
N       = 201j
xg      = np.mgrid[-10000:10000:N]
yg      = np.mgrid[-10000:10000:N]
nArr    = np.zeros((xg.size, yg.size))

nArr    = count_points(nArr, xg, yg, xin, yin, dx=(xg[1]-xg[0])/2.)
Nt      = xin.size
rArr    = np.arange(100000)*0.1+0.1
plotx, ploty = np.mgrid[-10000:10000:N, -10000:10000:N]
...
ax = plt.subplot(223)
plt.plot(xg, nArr[100,:], 'r-', lw=3, label='N, y = 0 nm')
plt.plot(yg, nArr[:, 100], 'k--', lw=3, label='N, x = 0 nm')
...
aa[nArr>5] = 1.
plt.pcolormesh(plotx, ploty, aa, shading='gouraud', vmax=1, vmin=0.)
plt.xlabel('X (nm)', fontsize=10)
plt.ylabel('Y (nm)', fontsize=10)
plt.axis([-10000, 10000, -10000, 10000], 'scaled')
cb=plt.colorbar()

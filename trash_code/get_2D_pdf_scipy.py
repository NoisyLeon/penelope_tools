...
ax      = plt.subplot(221)
plt.pcolormesh(plotx, ploty, zpdf, shading='gouraud', vmax=0.05, vmin=0.)
...
plotx, ploty = np.mgrid[-1e-5:1e-5:N, -1e-5:1e-5:N]
plt.pcolormesh(plotx, ploty, zpdf2, shading='gouraud', vmax=0.05, vmin=0.)
# 
plt.xlabel('X (cm)', fontsize=10)
plt.ylabel('Y (cm)', fontsize=10)
plt.axis([-1e-5, 1e-5, -1e-5, 1e-5], 'scaled')
...
outfname = outdir+'/'+pfx+str(z0)+'.pdf'

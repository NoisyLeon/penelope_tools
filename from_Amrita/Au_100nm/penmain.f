...
C
      open(35,FILE='psf-test.dat') !AVM 06/09/2017
  101 CONTINUE
      CALL SHOWER
...
C  ****  Write partial results after each dumping period.
        IF(LDUMP) THEN
          IF(TSEC-TSECAD.GT.DUMPP) THEN
            ...
            CALL PMWRT(-1)
            WRITE(6,1001) SHN
 1001       FORMAT(3X,'Number of simulated showers TEST =',1P,E14.7)
            CALL TIMER(TSEC)
            TSECAD=TSEC
            CPUT0=CPUTIM()
            GO TO 101
...
C
  ...
  103 CONTINUE
      CALL PMWRT(1)
      WRITE(6,1002) SHN
 1002 FORMAT(3X,'Number of simulated showers TEST =',1P,E14.7,
     1  /3X,'*** END ***')
C
      WRITE(26,'(/3X,''***  END  ***'')')
      WRITE(26,'(/3X,72(''-''))')
...
C
  302 CONTINUE
      CALL LOCATE
C      write(35,*) KPAR,E,X,Y,Z,U,V,W,WGHT,ILB(1),ILB(2),ILB(3),ILB(4),NSHI		!AVM 06/09/2017
C  781 format (i2,1p,8e13.5,i3,i2,i2,i9,i9,2i3) 						
C     write(6,'(''n,kpar,gen,x,y,z,w,e,ibody='',3i3,1p,5e11.3,0p,i3)')
C    1    MOD(N,100),KPAR,ILB(1),X,Y,Z,W,E,IBODY
C
      IF(MAT.EQ.0) THEN
...
C
  202 CONTINUE
      CALL SECPAR(LEFT)
      IF(LEFT.GT.0) THEN
      IF(KPAR.EQ.2) THEN			!AVM 06/09/2017
      write(35,*) KPAR,E,X,Y,Z,U,V,W,WGHT,
     1 ILB(1),ILB(2),ILB(3),ILB(4),NSHI		!AVM 06/09/2017
      ENDIF					!AVM 06/09/2017
